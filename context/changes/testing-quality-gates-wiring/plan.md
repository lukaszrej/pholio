...
- [ ] 1.1 CI workflow YAML valid — run is created without "invalid workflow file"
- [ ] 1.2 `unit` job runs `npm test` and passes on a green commit
- [ ] 1.3 `integration` job reaches and passes `npm run test:integration` (Supabase health check OK)
- [ ] 1.4 A broken unit assertion fails the `unit` job and the overall CI workflow

#### Manual

- [ ] 1.5 Integration logs show `supabase start` + migrations applied
- [ ] 1.6 `.env.test` populated with the three expected vars
- [ ] 1.7 Total CI wall-clock acceptable (unit fast; integration ~2–4 min)
