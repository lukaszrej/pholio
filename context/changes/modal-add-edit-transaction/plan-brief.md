...
`DashboardView.tsx` manages a `Set<string>` called `expandedTickers`; clicking a position row toggles it, rendering an inline `<tr colSpan={9}>` with a nested table of lots. The Edit and Delete buttons in that table already open the existing Dialog and AlertDialog modals, so only the *discovery surface* (the inline table itself) is changing.
